...
fun TradingScreen(repository: TradingRepository, marketData: ImportedMarketData? = null) {
    ...
    Column(modifier = Modifier.fillMaxSize().padding(12.dp), verticalArrangement = Arrangement.spacedBy(10.dp)) {
        Text("交易记录与复盘", style = MaterialTheme.typography.titleLarge)
        TabRow(selectedTabIndex = ui.tab.ordinal) {
            TradingTab.entries.forEachIndexed { index, tab ->
                Tab(selected = ui.tab == tab, onClick = { ui = ui.copy(tab = tab) }, text = { Text(tabLabel(tab)) })
            }
        }
        if (ui.error.isNotBlank()) {
            OutlinedCard(modifier = Modifier.fillMaxWidth()) {
                Text("错误：${ui.error}", color = MaterialTheme.colorScheme.error, modifier = Modifier.padding(12.dp))
            }
        }
        when (ui.tab) {
...
private fun TradesTab(
    ...
    LazyColumn(verticalArrangement = Arrangement.spacedBy(8.dp)) {
        ...
        items(trades.filter { ui.filter.matches(it) }, key = { it.trade.id }) { view ->
            TradeRow(
                view = view,
...
private fun EntryForm(
    ...
    OutlinedCard(modifier = Modifier.fillMaxWidth()) {
        Column(modifier = Modifier.padding(12.dp), verticalArrangement = Arrangement.spacedBy(8.dp)) {
            Text(if (editingTradeId == null) "录入交易" else "修订交易（原记录保留为已修订）", style = MaterialTheme.typography.titleMedium)
            OutlinedTextField(
...
private fun StrategySelector(
    ...
    Row(modifier = Modifier.fillMaxWidth(), horizontalArrangement = Arrangement.SpaceBetween) {
        Text("策略归属", style = MaterialTheme.typography.labelLarge)
...
private fun TradeRow(
    ...
    val sideColor = if (trade.side == TradeSide.BUY) Color(0xFF1B9E77) else Color(0xFFD1495B)
    OutlinedCard(modifier = Modifier.fillMaxWidth()) {
        Column(modifier = Modifier.padding(12.dp), verticalArrangement = Arrangement.spacedBy(4.dp)) {
            Row(modifier = Modifier.fillMaxWidth(), horizontalArrangement = Arrangement.SpaceBetween) {
                Text("${trade.instrumentId} · ${if (trade.side == TradeSide.BUY) "买入" else "卖出"}", fontWeight = FontWeight.Bold, color = sideColor)
                Text(if (selected) "已选择" else trade.status, style = MaterialTheme.typography.labelSmall)
            }
            Text("数量 ${trade.quantity} · 价格 ${trade.price} · 费用 ${view.totalFee}", style = MaterialTheme.typography.bodySmall)
            Text("时间 ${java.time.Instant.ofEpochMilli(trade.executedAtEpochMillis)}", style = MaterialTheme.typography.bodySmall)
...
private fun PositionsTab(positions: List<PositionView>) {
    if (positions.isEmpty()) {
        Text("当前无持仓", modifier = Modifier.padding(16.dp))
        return
    }
    LazyColumn(verticalArrangement = Arrangement.spacedBy(8.dp)) {
        items(positions, key = { it.instrumentId }) { position ->
            OutlinedCard(modifier = Modifier.fillMaxWidth()) {
                Column(modifier = Modifier.padding(12.dp)) {
                    Text(position.instrumentId, fontWeight = FontWeight.Bold)
...
private fun StatsTab(stats: TradingStatsResult?) {
    if (stats == null) {
        Text("暂无统计（需要先录入交易和收盘价）", modifier = Modifier.padding(16.dp))
        return
    }
    LazyColumn(verticalArrangement = Arrangement.spacedBy(8.dp)) {
        item {
            Row(horizontalArrangement = Arrangement.spacedBy(8.dp), modifier = Modifier.fillMaxWidth()) {
                StatCard("总收益率", "${"%.2f".format(stats.totalReturnPct)}%", Modifier.weight(1f))
                StatCard("最大回撤", "${"%.2f".format(stats.maxDrawdownPct)}%", Modifier.weight(1f))
            }
        }
        item {
            Row(horizontalArrangement = Arrangement.spacedBy(8.dp), modifier = Modifier.fillMaxWidth()) {
                StatCard("胜率", "${"%.1f".format(stats.winRatePct)}%", Modifier.weight(1f))
                StatCard("盈亏比", stats.profitFactor?.let { "%.2f".format(it) } ?: "无亏损样本", Modifier.weight(1f))
            }
        }
        item {
            Row(horizontalArrangement = Arrangement.spacedBy(8.dp), modifier = Modifier.fillMaxWidth()) {
                StatCard("平均暴露", "${"%.1f".format(stats.averageExposurePct)}%", Modifier.weight(1f))
                StatCard("最大暴露", "${"%.1f".format(stats.maxExposurePct)}%", Modifier.weight(1f))
            }
        }
        item {
            OutlinedCard(modifier = Modifier.fillMaxWidth()) {
                Column(modifier = Modifier.padding(12.dp), verticalArrangement = Arrangement.spacedBy(4.dp)) {
                    Text("归因（已实现）", fontWeight = FontWeight.Bold)
                    stats.realizedByStrategy.toSortedMap().forEach { (key, value) -> Text("策略 $key：${"%.2f".format(value)}") }
                    stats.realizedByInstrument.toSortedMap().forEach { (key, value) -> Text("标的 $key：${"%.2f".format(value)}") }
                    Text("归因（未实现）", fontWeight = FontWeight.Bold)
                    stats.unrealizedByStrategy.toSortedMap().forEach { (key, value) -> Text("策略 $key：${"%.2f".format(value)}") }
                    stats.unrealizedByInstrument.toSortedMap().forEach { (key, value) -> Text("标的 $key：${"%.2f".format(value)}") }
                }
            }
        }
        item { NavChart(stats) }
...
private fun NavChart(stats: TradingStatsResult) {
    OutlinedCard(modifier = Modifier.fillMaxWidth()) {
        Column(modifier = Modifier.padding(12.dp)) {
            Text("净值曲线（含现金）", fontWeight = FontWeight.Bold)
            val points = stats.navCurve
            if (points.size < 2) {
                Text("净值数据不足", style = MaterialTheme.typography.bodySmall)
            } else {
                Canvas(modifier = Modifier.fillMaxWidth().height(180.dp)) {
                    val minNav = points.minOf { it.nav }
                    val maxNav = points.maxOf { it.nav }
                    val range = (maxNav - minNav).takeIf { it > 0 } ?: 1.0
                    val stepX = size.width / (points.size - 1)
                    val path = Path()
                    points.forEachIndexed { index, point ->
                        val x = index * stepX
                        val y = size.height - ((point.nav - minNav) / range * size.height).toFloat()
                        if (index == 0) path.moveTo(x, y) else path.lineTo(x, y)
                    }
                    drawPath(path, color = Color(0xFF2563EB), style = androidx.compose.ui.graphics.drawscope.Stroke(width = 3f))
                    drawLine(Color(0xFF9CA3AF), Offset(0f, size.height), Offset(size.width, size.height), strokeWidth = 1f)
                }
            }
        }
    }
...
private fun ReviewTab(
    ...
    LazyColumn(verticalArrangement = Arrangement.spacedBy(8.dp)) {
        item { if (stats != null) NavChart(stats) else Text("暂无净值数据", modifier = Modifier.padding(8.dp)) }
        item {
            OutlinedCard(modifier = Modifier.fillMaxWidth()) {
                Column(modifier = Modifier.padding(12.dp), verticalArrangement = Arrangement.spacedBy(8.dp)) {
                    Text("个人数据备份与恢复", fontWeight = FontWeight.Bold)
                    OutlinedTextField(
                    ...
                    if (backup.message.isNotBlank()) {
                        Text(backup.message, color = Color(0xFF1B9E77))
                        TextButton(onClick = onDismissMessage) { Text("知道了") }
                    }
                    if (backup.error.isNotBlank()) {
                        Text(backup.error, color = MaterialTheme.colorScheme.error)
                        TextButton(onClick = onDismissMessage) { Text("知道了") }
                    }
                    Text("恢复过程在事务中完成：任何校验失败都不会改动现有数据。", style = MaterialTheme.typography.bodySmall)
                }
            }
        }
    }
