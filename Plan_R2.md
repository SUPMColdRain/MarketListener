...
- `type`：性能与前端基础设施；`priority`：P0；`执行对象/模块`：Vue router、`domain/api.ts`、视图、测试；`state`：DONE；`failure_count`：2；`来源`：R2 总指令。
...
- `实际修改文件`：`desktop/src/market_monitor/web_api/watchlist.py`、`desktop/web/src/views/DataView.vue`、`desktop/web/src/styles.css`、`desktop/web/src/components/charts/SeriesChart.vue`、`desktop/src/market_monitor/web_api/dashboard.py`、`desktop/src/market_monitor/market_breadth.py`、`desktop/src/market_monitor/collector.py`、`desktop/tests/test_web_dashboard_api.py`、`desktop/tests/test_market_breadth.py`；`验证命令`：定向 dashboard/market-breadth pytest、`npm run build`、`npm run test:e2e`、本机 `catalog.duckdb` 查询；`验证结果`：个人面板配置仅写入受 Pydantic 约束的本机 JSON，支持新增、删除、隐藏/恢复、排序、半宽/整行布局、标题、折线/柱状图、颜色、透明度与时间范围；缺失宽度字段的旧布局默认半宽，定向测试和 14 项端到端测试通过。广度只统计 A 股个股，移除了 ETF/指数混入和所有 `±9.9%` 近似涨跌停。A 股涨停池独立面板仅读取东财权威 `CN_ZT_POOL` 的涨停/跌停家数、最高连板和昨日涨停接盘收益率，本机验证为 2026-08-07 的 5 条真实记录；测试确保近似指标不会混入。金银比增加独立面板，仅从 `GOLD_SILVER_RATIO` 的同日 COMEX GC/SI 本地派生记录读取；本机存在 2,590 条（2016-08-08 至 2026-08-07）可展示记录。F10 直达详情增加一次只读短暂重试，修复并发页面巡检下的短时连接失败，完整端到端复验通过。旧涨停/跌停热力图断言在移除未验证统一阈值后失败一次，`failure_count=1`，已改为只验证上涨/下跌/平盘后通过；`遗留问题`：权威池的未来增量持续可用性由外部上游决定，失败时页面诚实为空态。

### R2-T004 — 行情页面分类、周期与按面板加载重构

- `type`：产品与性能；`priority`：P0；`执行对象/模块`：Market API、聚合、MarketView、图表；`state`：DONE；`failure_count`：0；`来源`：R2 总指令、R1-T004/T006。
...
- `type`：数据工程；`priority`：P1；`执行对象/模块`：Provider、探针、Silver、catalog、文档；`state`：BLOCKED；`failure_count`：2；`来源`：R2 总指令、R1-T007。
