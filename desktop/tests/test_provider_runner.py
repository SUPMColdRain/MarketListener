...
@pytest.mark.parametrize(
    ...
    [
        ([Capability("normal", CapabilityStatus.PASS, row_count=2)], "PASS", None),
        ...
        (
            [
                Capability("partial", CapabilityStatus.PASS, row_count=1),
                Capability("missing", CapabilityStatus.FAILED, "field absent"),
            ],
            "FAILED",
            None,
        ),
...
def test_fixed_provider_responses_are_classified(
    ...
    assert (result.error.category.value if result.error else None) == expected_category
...
def test_failure_does_not_stop_later_provider_and_reports_are_dual_format(tmp_path) -> None:
    report = ProbeRunner().run(
        [
            ...
            StaticProvider("healthy", [Capability("bars", CapabilityStatus.PASS, row_count=2)]),
        ]
    )
    ...
    assert [item["provider"] for item in payload["providers"]] == ["broken", "healthy"]
    assert payload["providers"][0]["error"]["message"] == "token=***"
    assert "healthy: PASS" in human_path.read_text(encoding="utf-8")
...
def test_timeout_is_reported_and_does_not_stop_later_provider() -> None:
    ...
    try:
        report = ProbeRunner(timeout_seconds=0.01).run(
            [blocking, StaticProvider("healthy", [Capability("bars", CapabilityStatus.PASS, row_count=2)])]
        )
    finally:
        blocking.release.set()

    assert monotonic() - started < 1
    assert [result.status for result in report.results] == [CapabilityStatus.FAILED, CapabilityStatus.PASS]
    assert report.results[0].error == ProviderError(ErrorCategory.NETWORK, "provider probe exceeded 0.01 seconds")
